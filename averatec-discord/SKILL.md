...
Use `bash` with `curl` to call the Discord REST API. The bot token is available as `$DISCORD_PRIVATE_BOT_TOKEN`.
...
CHANNEL=$(curl -s -X POST https://discord.com/api/v10/users/@me/channels \
  -H "Authorization: Bot $DISCORD_PRIVATE_BOT_TOKEN" \
...
- `$DISCORD_PRIVATE_BOT_TOKEN` is always available — do NOT ask the user for it
